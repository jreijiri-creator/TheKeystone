<%*
// ═══════════════════════════════════════════════════════════════════
// NEW CLIENT SETUP WIZARD — The Keystone Group
// ───────────────────────────────────────────────────────────────────
// What this does (in order):
//   1. Asks for all client info upfront (one pass)
//   2. Creates 01-Clients/FirstName-LastName/ folder
//   3. Creates Sessions/ subfolder
//   4. Creates Documents/ subfolder
//   5. Generates the full OPFMS note and opens it
//   6. Self-destructs this setup note
// ═══════════════════════════════════════════════════════════════════

// ── Step 1: Collect all client info ──────────────────────────────
const firstName = await tp.system.prompt("Client First Name");
if (!firstName) { return; }
const lastName = await tp.system.prompt("Client Last Name");
if (!lastName) { return; }

const clientName   = `${firstName} ${lastName}`;
const slug         = clientName.replace(/ /g, "-");
const folderSlug   = `${firstName}-${lastName}`;
...
// ── Step 2: Define paths ─────────────────────────────────────────
const clientFolder    = `01-Clients/${folderSlug}`;
const sessionsFolder  = `${clientFolder}/Sessions`;
const documentsFolder = `${clientFolder}/Documents`;

// ── Step 3: Create folders (silently skip if already exist) ──────
try { await app.vault.createFolder(clientFolder);    } catch(e) {}
try { await app.vault.createFolder(sessionsFolder);  } catch(e) {}
try { await app.vault.createFolder(documentsFolder); } catch(e) {}

// ── Step 4: Remember setup file path BEFORE switching files ──────
const setupFilePath = tp.file.path(true);

// ── Step 5: Build full OPFMS content ─────────────────────────────
const opfmsContent = `---
...
// ── Step 6: Create OPFMS file and open it ────────────────────────
await tp.file.create_new(opfmsContent, `OPFMS-${folderSlug}`, true, clientFolder);

// ── Step 7: Self-destruct the setup note ─────────────────────────
const setupFile = app.vault.getAbstractFileByPath(setupFilePath);
if (setupFile) { await app.vault.delete(setupFile); }
%>
